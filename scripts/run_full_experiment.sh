...
PROJECT_ROOT="${PROJECT_ROOT:-$HOME/GMY}"
...
conda run -n adacolrag-core python scripts/download_vidore_v3.py \
...
conda run -n adacolrag-colpali python scripts/export_colvision.py \
...
conda run -n adacolrag-visrag python scripts/export_visrag.py \
  --dataset-dir "$DATASET_DIR" \
...
conda run -n adacolrag-core python scripts/run_matrix.py \
  --dataset-dir "$DATASET_DIR" \
...
set +e
conda run -n adacolrag-core python scripts/compare_results.py \
